...
# Filter the data to keep only players who competed in all events and have a non-zero "Pick" value
...
data_array = filtered_data.to_numpy()

# Perform under sampling
rus = RandomUnderSampler(random_state=42)
X_resampled, Y_resampled = rus.fit_resample(data_array[:, :-1], data_array[:, -1])

# Build the decision tree based on the resampled data
attrs = list(range(X_resampled.shape[1]))  # Exclude the target column
tree = buildTree(np.column_stack((X_resampled, Y_resampled)), attrs)

# Split the resampled dataset into training and testing sets
X_train, X_test, Y_train, Y_test = split_dataset(np.column_stack((X_resampled, Y_resampled)), test_size=0.1, rand_seed=42)
...
# # Build the decision tree based on the under-sampled data
# attrs = list(range(X_resampled.shape[1]))  # Exclude the target column
# tree = buildTree(np.column_stack((X_resampled, Y_resampled)), attrs)

# # Split the under-sampled dataset into training and testing sets
# X_train, X_test, Y_train, Y_test = split_dataset(np.column_stack((X_resampled, Y_resampled)), test_size=0.1, rand_seed=42)
